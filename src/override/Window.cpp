...
// Real Retina scale factor; see CardinalCommon.hpp for the rationale.
...
void Window::step() {
	...
	// a screen, so it returns 1.0 default and stays stuck. cardinalActualScaleFactor
	// queries NSScreen at runtime to get the real value (2.0 on Retina).
	// CardinalUI.cpp's mouse handlers use the same helper, so the scene size
	// (computed from pixelRatio) and mouse coords stay in the same coordinate
	// space.
	float newPixelRatio = (float)cardinalActualScaleFactor(internal->tlw->getScaleFactor());
